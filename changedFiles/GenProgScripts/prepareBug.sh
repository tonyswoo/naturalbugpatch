...
if [[ "$LOWERCASEPACKAGE" == "mockito"  && ( $BUGNUMBER -le 9 || ( $BUGNUMBER -ge 18 && $BUGNUMBER -le 20 ) ) ]] ; then
	rm $BUGWD/buildSrc/src/"test"/groovy/org/mockito/release/notes/NotesPrinterTest.groovy
	if [ $BUGNUMBER -ne 9 ]; then
...
elif [ $LOWERCASEPACKAGE$BUGNUMBER == "closure106" ]; then
	cp $D4J_HOME/framework/projects/Closure/lib/junit.jar $BUGWD/lib/junit.jar
fi

if [ $FAULTLOC == "true" ]; then
	FAULTLOC="humanInjected"
	cp $HOME/faultLoc/$PROJECT$BUGNUMBER"FaultLoc" $BUGWD/$PROJECT$BUGNUMBER"FaultLoc"
else
	FAULTLOC="standardPathFile"
fi
...
continue=true
useEntropy=$ENTROPY
faultLocStrategy=$FAULTLOC
pathToFileHumanInjectedFaultLoc=$BUGWD/$PROJECT${BUGNUMBER}FaultLoc
